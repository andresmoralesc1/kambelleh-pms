...
import { SprayCan, CheckCircle, AlertCircle, Clock, Wrench, ChevronDown, ChevronUp } from 'lucide-react';
import { useRooms, useRoomCleaningLogs, useUpdateRoomCleaningStatus, useCreateCleaningLog } from '../hooks/useQueries';
...
export default function Housekeeping() {
  ...
  const handleMarkClean = (room) => {
    setConfirmState({
      ...
      message: `¿Registrar limpieza completada para la habitación #${room.number}?`,
      confirmLabel: 'Marcar limpia',
      resolve: async (ok) => {
        if (!ok) return;
        try {
          await createLog.mutateAsync({
            roomId: room.id,
            data: {
              performedBy: 'Personal de limpieza',
              status: 'CLEANED',
      ...
      {recentLogs.map(log => {
        ...
        return (
          <div key={log.id} className="flex items-center gap-2 p-2 rounded-lg bg-white border border-surface-200">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-surface-700 truncate">{log.performedBy}</p>
