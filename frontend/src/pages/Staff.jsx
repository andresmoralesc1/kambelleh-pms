...
const roleConfig = {
  ...
  MANAGER: { label: 'Gerencia', bg: 'bg-blue-100', text: 'text-blue-700' },
  RECEPTIONIST: { label: 'Recepción', bg: 'bg-emerald-100', text: 'text-emerald-700' },
};
...
function UserModal({ user, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" role="dialog" aria-modal="true" aria-labelledby="user-modal-title">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 id="user-modal-title" className="text-lg font-bold text-surface-900">{isEditing ? 'Editar' : 'Nuevo'} usuario</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-surface-700 mb-1">Rol *</label>
          ...
          <div>
            <label className="block text-xs font-medium text-surface-700 mb-1">
              Contraseña {isEditing && <span className="text-surface-400">(solo si deseas cambiarla)</span>}
            </label>
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-surface-500">{totalReservations} reserva{totalReservations !== 1 ? 's' : ''} creada{totalReservations !== 1 ? 's' : ''}</p>
          </div>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={() => onChangePassword(user)}
            aria-label={`Cambiar contraseña de ${user.name}`}
            className="p-1.5 rounded-lg hover:bg-blue-50 text-surface-500 hover:text-blue-600 transition-colors"
  ...
  return (
    <div className="p-6 space-y-5" role="main">
      ...
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-surface-900">Personal</h1>
          <p className="text-surface-500 text-sm mt-0.5">{users.length} usuarios registrados</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <h3 className="font-semibold text-surface-700 mb-1">Sin usuarios</h3>
